...
_Swagger UI Live Demo page showcasing real-time testing of API endpoints in an interactive environment_
...
_The 'Create API' page on Swagger UI where users can begin documenting a new API project_

> For this tutorial, we will be working with a sample Petstore API. However, if you already have an API to document, Click on **Import API** > **Note: Swagger only permits API in OpenAPI JSON or YAML format**
...
_Form interface in Swagger UI for entering basic information about the Petstore API._
...
2. The alternative to \***\*`**Code**`is`**Form**`. With `**Form\*\*`, you can directly fill in the parameters and documentation as text fields.
   ...
   _Comparison of Swagger UI's code editor and form view, allowing users to choose between writing YAML or using form fields_
...
_Preview of the documented Petstore API in Swagger UI with an interactive display of endpoints and schemas_
...
- Click on **Try it out** under each request and see the real-time response below.
  ...
  _The 'Try it out' button in Swagger UI_

- Click on **Execute** to see the real-time response
  ![Execute Button and Response in Swagger](./img/execute-button.png)
  _The "Execute" button in Swagger UI, which sends the request and displays the response below_
...
_The Image of how your API documentation site should look like._
